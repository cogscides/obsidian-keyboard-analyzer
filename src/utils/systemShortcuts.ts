...
export function getSystemShortcutCommands(): commandEntry[] {
  ...
  const common: { id: string; name: string; hotkeys: hotkeyEntry[] }[] = [
    ...
    // Redo: macOS commonly uses Shift+Cmd+Z, Windows/Linux often Ctrl+Y
    {
      id: 'system:redo',
      name: 'Redo',
      hotkeys: isMac() ? [hk(['Mod', 'Shift'], 'z')] : [hk(['Control'], 'y')],
    },
  ...
  if (isWindows() || isLinux()) {
    const winLin: { id: string; name: string; hotkeys: hotkeyEntry[] }[] = [
      {
        id: 'system:copy-insert',
        name: 'Copy (Ctrl+Insert)',
        hotkeys: [hk(['Control'], 'Insert')],
      },
      {
        id: 'system:paste-shift-insert',
        name: 'Paste (Shift+Insert)',
        hotkeys: [hk(['Shift'], 'Insert')],
      },
      {
        id: 'system:cut-shift-delete',
        name: 'Cut (Shift+Delete)',
        hotkeys: [hk(['Shift'], 'Delete')],
      },
      // Selection / navigation examples
      {
        id: 'system:select-char-left',
        name: 'Select Left',
        hotkeys: [hk(['Shift'], 'ArrowLeft')],
      },
      {
        id: 'system:select-word-left',
        name: 'Select Word Left',
        hotkeys: [hk(['Control', 'Shift'], 'ArrowLeft')],
      },
      {
        id: 'system:select-word-right',
        name: 'Select Word Right',
        hotkeys: [hk(['Control', 'Shift'], 'ArrowRight')],
      },
    ]
  ...
  if (isMac()) {
    const mac: { id: string; name: string; hotkeys: hotkeyEntry[] }[] = [
      {
        id: 'system:move-word-left',
        name: 'Move Word Left',
        hotkeys: [hk(['Alt'], 'ArrowLeft')],
      },
      {
        id: 'system:select-word-left-mac',
        name: 'Select Word Left',
        hotkeys: [hk(['Alt', 'Shift'], 'ArrowLeft')],
      },
      {
        id: 'system:select-word-right-mac',
        name: 'Select Word Right',
        hotkeys: [hk(['Alt', 'Shift'], 'ArrowRight')],
      },
    ]
    for (const c of mac) {
      items.push({
