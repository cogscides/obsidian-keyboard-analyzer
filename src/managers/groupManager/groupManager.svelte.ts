...
export default class GroupManager {
  ...
  public hasUnsavedFilterChanges(groupId: string): boolean {
    try {
      if (groupId === GroupType.All) {
        ...
        // If no defaults are saved yet, consider it as no unsaved changes
      ...
      // Compare current filter settings with group's defaults
      const currentFilters = group.filterSettings
      const defaultFilters = group.defaults?.filters

      // If no defaults are saved, consider current as default (no unsaved changes)
      // This handles legacy groups that were created before the defaults system
      if (!defaultFilters) return false

      return !this.isEqualFilters(currentFilters, defaultFilters)
    } catch (err) {
      logger.error('[groups] hasUnsavedFilterChanges failed', { groupId, err })
      return false
    }
  }
  ...
  public saveCurrentFiltersAsDefaults(groupId: string): void {
    try {
      ...
      const group = this.getGroup(groupId)
      if (!group) {
        logger.warn('[groups] saveCurrentFiltersAsDefaults: group not found', {
          groupId,
        })
        return
      }
      ...
      this.setGroupDefaults(groupId, {
        filters: { ...group.filterSettings },
      })

      logger.debug('[groups] saved current filters as defaults', {
        groupId,
        filters: group.filterSettings,
      })
    } catch (err) {
      logger.error('[groups] saveCurrentFiltersAsDefaults failed', {
        groupId,
        err,
      })
    }
  ...
  public resetFiltersToDefaults(groupId: string): void {
    try {
      if (groupId === GroupType.All) {
        ...
        if (savedDefaults) {
          this.settingsManager.updateSettings({
            defaultFilterSettings: { ...savedDefaults },
          })
        }
        return
      }
      ...
      const group = this.getGroup(groupId)
      if (!group?.defaults?.filters) {
        logger.warn('[groups] resetFiltersToDefaults: no defaults found', {
          groupId,
        })
        return
      }

      // Apply saved defaults to current filters
      this.applyDefaultsToGroupFilters(groupId)

      logger.debug('[groups] reset filters to defaults', {
        groupId,
        defaults: group.defaults.filters,
      })
    } catch (err) {
      logger.error('[groups] resetFiltersToDefaults failed', { groupId, err })
    }
  }
