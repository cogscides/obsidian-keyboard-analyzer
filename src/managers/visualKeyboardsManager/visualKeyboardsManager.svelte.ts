...
export class VisualKeyboardManager {
	...
	private codeToObsidianModifier: Record<
		string,
	...
	public mapCodeToObsidianModifier(
		codeOrLabel: string,
	): ("Control" | "Alt" | "Shift" | "Meta") | undefined {
		// Prefer direct code lookup
		if (this.codeToObsidianModifier[codeOrLabel])
			return this.codeToObsidianModifier[codeOrLabel];
		// Try normalized by capitalized code form
		const norm = codeOrLabel;
		if (this.codeToObsidianModifier[norm])
			return this.codeToObsidianModifier[norm];
		...
		for (const command of cmds) {
			...
			for (const hotkey of hkList) {
				...
				if (strictModifierMatch && hasActive) {
					...
					const keyMatch =
						!activeKey || isKeyMatch(activeKey, hotkey.key || "");
	...
	public updateVisualState(activeKey: string, activeModifiers: Modifier[]) {
		...
		const abstractActiveModifiers = new Set(
			unconvertModifiers(activeModifiers),
		);
	...
	public previewHoverState(previewKey: string, previewModifiers: string[]) {
		...
		const abstractMods = new Set(
			(previewModifiers || []).map((m) =>
				convertModifier(m as unknown as Modifier),
			),
		);
